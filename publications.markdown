...
These articles are dedicated to the problem of finding polynomials / rational functions with separated variables. The first explains how to determine $I \cap \left(\mathbb{C}[x] + \mathbb{C}[y]\right)$ when $I$ is an ideal in a bivariate polynomial ring $\mathbb{C}[x,y]$ given in terms of a bases. The second and third continue the ideas developed therein and study equations of the form $r(x,y) + q(x,y) p(x,y) = f(x) - g(y)$ for rational functions $r$, $q$, $p$ and $f$ and $g$ of which only the rational function $r$ and the polynnomial $p$ is known. The latter two problems have a field theoretic interpretation and relate to (the simplest instance of) the problem of computing intersections of fields and finding decompositions of elements of thereof.         
